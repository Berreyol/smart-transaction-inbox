...
A React Native (Expo) + Supabase app. Users forward bank notification emails to a global inbound address generated by a Pipedream workflow (trigger: Email); a step in that workflow POSTs the parsed email to a Supabase Edge Function, which regex-parses the body, matches the sender to a user by email, and drops a row into `pending_transactions` for in-app approval. Full flow is diagrammed in `README.md` — read that first for the "why," this file is the "how to work in the code."
